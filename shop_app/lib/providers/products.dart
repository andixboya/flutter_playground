...
class Products with ChangeNotifier {
  ...
  // 272) userId is necessary for filtering all of the products. It is passed from auth service
  final String userId;
...
// 204-207) favorites are added here for filter.
    ...
    try {
      final response = await http.post(url,
          ...
          body: json.encode({
            ...
            'imageUrl': product.imageUrl,
            'price': product.price,
            // 273) creator is added as info, only on the server and is later used for filtering of products.
            'creatorId': userId,
  ...
  // 273) filter check is added (by user)
  Future<void> fetchAndSetProducts([bool filterByUser = false]) async {
    // 273) it is triggered server side.
    final filterString =
        filterByUser ? 'orderBy="creatorId"&equalTo="$userId"' : '';
    //  270) here the token is added the access token is accessed through access_token not auth like in the vid.
    // finally done!
    var url = Uri.parse(
        'https://flutter-shop-app-6b626-default-rtdb.firebaseio.com/products.json?auth=$authToken&$filterString');
    try {
      final response = await http.get(url);
      // 272) here the fav. is loaded from separate colelction, so it needs to be matched!
      url = Uri.https('flutter-shop-app-6b626-default-rtdb.firebaseio.com',
      ...
      extractedMap.forEach((prodId, prodData) {
        loadedProducts.add(
          ...
          // 272) here the fav is set throguh another req.
          isFavorite:
              // alternate syntax for null (you know it)!
